...
import { Dialog, Input, Button, Alert } from 'tdesign-react';
...
export default function SampleChat(props: ISampleChatProps) {
  ...
  const [SDKSecretKey, setSDKSecretKey] = useState('');
  ...
  const init = () => {
    if(!sessionStorage.getItem("SECRETKEY") || ! sessionStorage.getItem("SDKAPPID")) {
      setDialogShow(true);
      return;
    }
  ...
  const bodyConfirm = () => {
    if(!SDKAppId || !SDKSecretKey) {
      alert('Please enter SDKAppID and SDKSecretKey');
      return;
    }
    sessionStorage.setItem("SECRETKEY", SDKSecretKey);
  ...
  return (
    <div className="sample-demo">
      ...
      >
        <div>
          <div style={{marginBottom: '5px'}}>SDKAppID</div>
          <Input
            onChange={(value) => {setSDKAppId(value)}}
            placeholder="Please input SDKAppID"
          />
          <div style={{marginTop: '18px', marginBottom: '5px'}}> SDKSecretKey</div>
          <Input
            style={{marginBottom: '18px'}}
            onChange={(value) => {setSDKSecretKey(value)}}
            placeholder="Please input SDKSecretKey"
          />
          <Alert 
            theme="warning"
            message={<div> The SDKAppID and SDKSecretKey can be found in the <a href="https://console.trtc.io/overview" target="_blank">TRTC console↗</a>. Please refer to the <a href="https://trtc.io/document/35166?platform=web&product=conference#.E5.AE.A2.E6.88.B7.E7.AB.AF.E7.A4.BA.E4.BE.8B.E4.BB.A3.E7.A0.81.E8.AE.A1.E7.AE.97-UserSig">document</a> for how to obtain it.</div>} 
          />
        </div>
      </Dialog>
      <div className="chat-header">
        ...
        {(!sessionStorage.getItem("SECRETKEY") || !sessionStorage.getItem("SDKAPPID")) ? 
        <Button theme="primary" variant="text" onClick={() => {setDialogShow(true)}}>Login</Button> :
        <Button theme="primary" variant="text" onClick={() => {sessionStorage.removeItem('SECRETKEY'); sessionStorage.removeItem('SDKAPPID'); window.location.reload()}}>Logout</Button>
        }
      </div>
      {isPC ? (
